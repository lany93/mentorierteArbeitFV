...
\end{align}
    ...
    \begin{figure}[htp]
    \centering
    \includegraphics[width=10cm]{images/einleitung/WürfelE1.png}
    \caption{Projektion der Kanten des Würfels auf die Ebene $E_1$.}
    \label{fig:würfelE1}
    \end{figure}
...
\begin{itemize}
    ...
    \item Infolgedessen können wir jede Kante des Würfels als einen Vektor betrachten, indem wir ihren Fußpunkt in den Ursprung verschieben. 
\end{itemize}
    ...
    \begin{figure}[htp]
        ...
        \begin{subfigure}[b]{0.45\textwidth}
            \centering
            \includegraphics[width=\linewidth]{images/einleitung/OrthogonalprojektionVektor.png}
            \caption{Translation Vektor.}
            %\label{fig:Rhombenkuboktaeder}
        \end{subfigure}
        \hfill
        \begin{subfigure}[b]{0.45\textwidth}
            \centering
            \includegraphics[width=\linewidth]{images/einleitung/VerschiebungKoordinatensystem.png}
            \caption{Kante $\overline{BF}$ als Vektor.}
            %\label{fig:RhombenkuboktaederZerlegt}
        \end{subfigure}
        \caption{Beobachtungen zur Vereinfachung der Berechnung der projizierten Kantenlängen eines Würfels unter einer Orthogonalprojektionen.}
...
wobei $\vec{v}_{CG}$ den Vektor der Kante $\overline{CG}$ repräsentiert und $\alpha$ der Winkel zwischen diesem Vektor und dem Normalenvektor $\vec{n}_{E_1}$ ist.

    Mit dem Kantenvektor 
    \begin{align}
        \overline{CG}: \vec{v}_{CG}&=\begin{pmatrix} 0 \\ 0 \\ 1 \end{pmatrix},\|\vec{v}_{CG}\| = 1 \nonumber
    \end{align} sowie der Berechnung
    \begin{align}
...
Wir behalten dieses Ergebnis im Hinterkopf und betrachten nun die Orthogonalprojektion auf die Ebene $E_2$ mit dem Einheitsnormalenvektor
    ...
    \begin{figure}[htp]
    \centering
    \includegraphics[width=0.7\textwidth, keepaspectratio]{images/einleitung/WürfelE2.png}
    \caption{Projektion der Kanten des Würfels auf die Ebene $E_2$.}
    \label{fig:würfelE2}
    \end{figure}
...
\begin{itemize}
    ...
    \item Die restlichen acht Kanten schließen alle denselben Winkel mit dem Normalenvektor $\vec{n}_{E_2}$ ein, wie beispielsweise die Kante $\overline{GH}$.
\end{itemize}

Für die parallel zur Ebene verlaufenden Kanten gilt, dass ihre Länge bei der Projektion erhalten bleibt:
\begin{align}
    \|\Pi_{E_2}(\vec{v}_{CG})\| = 1. \nonumber
\end{align}
    Für die anderen acht Kanten berechnen wir die Projektionslänge analog zu Gleichung \eqref{eq:einführungsbeispielprojectionsimple}. Da der Kantenvektor $\vec{v}_{GH} = (1, 0, 0)^T$ einen Winkel von $\alpha = 45^\circ$ ($\frac{\pi}{4}$) mit dem Normalenvektor einschließt, erhalten wir:
\begin{align}
...
\begin{align}
    \sum_{\vec{v}_k \in K_W} \|\Pi_{E_1}(\vec{v}_k)\|^2 = \sum_{\vec{v}_k \in K_W} \|\Pi_{E_2}(\vec{v}_k)\|^2. \nonumber
\end{align}
...
\begin{align}
    \sum_{\vec{v}_k \in K_W} \|\Pi_{E_1}(\vec{v}_k)\|^2 = 12 \cdot \left(\sqrt{\frac{2}{3}}\right)^2 = 12 \cdot \frac{2}{3} = 8, \nonumber
\end{align}
und für die Ebene $E_2$
\begin{align}
    \sum_{\vec{v}_k \in K_W} \|\Pi_{E_2}(\vec{v}_k)\|^2 = 4 \cdot 1^2 + 8 \cdot \left(\frac{\sqrt{2}}{2}\right)^2 = 4 + 8 \cdot \frac{1}{2} = 8. \nonumber
\end{align}
...
\begin{exercise} \label{ex:verificationeinstiegesbeispielEbene3}
    ...
    \begin{align}
        E_3:\ x = 0. \nonumber
    \end{align}
    Berechnen Sie hierzu die Summe der quadrierten Längen der orthogonal projizierten Kanten des regulären Hexaeders auf $E_3$.
\end{exercise}
