...
\begin{itemize}
    \item Acht Kanten liegen parallel zur Ebene $E_3$ (bzw. stehen orthogonal auf dem Normalenvektor $\vec{n}_{E_3}$), wie beispielsweise die Kante $\overline{CD}$.
    \item Die restlichen vier Kanten sind parallel zum Normalenvektor $\vec{n}_{E_3}$, wie beispielsweise die Kante $\overline{CG}$.
\end{itemize}
Somit folgt
\begin{align}
...
An dieser Stelle ist anzumerken, dass wir bei den Beispielen aus Alltag, Natur und Architektur eine Idealisierung vornehmen. Während ein mathematisches Polyeder exakt ebene Flächen und unendlich scharfe Kanten voraussetzt, weisen reale Objekte stets kleine Abweichungen an den Ecken auf.
...
\item[c)] Die Symmetriegruppe des regulären Oktaeders ist identisch zur Symmetriegruppe des regulären Hexaeders. 
\item[d)] Die Symmetriegruppe des regulären Dodekaeders ist identisch zur Symmetriegruppe des regulären Ikosaeders. 
\end{itemize}
...
Wir haben eine Menge $M$ und eine Äquivalenzrelation $\sim$. Eine Äquivalenzklasse eines Elements $a$ ist definiert als $$[a] := \{x \in M \mid x \sim a\}$$

Wir müssen nun zwei Dinge beweisen:
\begin{enumerate}
    \item Dass die Klassen zusammen die ganze Menge ergeben (Vereinigung).
    \item Dass sie streng disjunkt sind (sich nicht überschneiden, außer sie sind exakt gleich).
\end{enumerate}
\vspace{0.4cm}
Zu 1: Da die Äquivalenzrelation reflexiv ist, gilt $$ a \sim a$$
für jedes $a \in M$. Da das für jedes Element gilt, ergibt die Vereinigung aller Äquivalenzklassen zwangsläufig die komplette Menge $M$.

\vspace{0.4cm}
Zu 2.: Nehmen wir an, die Klassen überschneiden sich teilweise. Es gibt also ein Element $c$, das in beiden Klassen liegt ($c \in [a]$ und $c \in [b]$).  Es gilt also
$$ c \sim a \ \text{und} \ c \sim b.$$
Da die Äquivalenzrelation symmetrisch und transitiv ist, folgt
$$ a \sim b \implies [a] = [b].$$
Zwei Klassen können sich nicht nur teilweise überschneiden. 
\vspace{0.4cm}
Somit folgt, dass die Menge $M$ die disjunkte Vereinigung ihrer Äquivalenzklassen bezüglich $\sim$ ist.
